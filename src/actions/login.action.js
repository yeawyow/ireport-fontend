...
export const setStateToSuccess = (payload, accessToken) => ({
  type: LOGIN_SUCCESS,
  payload,
  accessToken,
});
...
export const login = ({ username, passwordjwt, history }) => {
  return async (dispatch) => {
    ...
    if (result.data.result == "ok") {
      ...
      dispatch(setStateToSuccess(result.data.result, result.data.token));
      history.push("/dashboard");
    } else {
      localStorage.setItem(LOGIN_STATUS, "nok");
      dispatch(setStateToFailed(result.data.result));
    }
